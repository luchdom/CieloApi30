...
namespace Api30.Services
{
    public class CieloEcommerceService
    {
        ...
        public async Task<Sale> QuerySale(string paymentId)
        {
            var querySaleRequest = new QuerySaleRequest(_merchant, _environment);
            return await querySaleRequest.ExecuteAsync(paymentId);
        }

        public async Task<Sale> CancelSale(string paymentId, double? amount = null)
        {
        ...
        public async Task<Sale> CaptureSale(string paymentId, decimal? amount = null, decimal? serviceTaxAmount = null)
        {
